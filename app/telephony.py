...
def twilio_voice(request: Request):
    ...
    vr = VoiceResponse()
    vr.say("Bitte sprechen Sie nach dem Signal. Drücken Sie die Raute, wenn Sie fertig sind.", language="de-DE")
    vr.record(max_length=60, action=str(request.url_for("twilio_recording")), play_beep=True, finish_on_key="#")
...
async def twilio_recording(request: Request, background_tasks: BackgroundTasks):
    ...
    def tts_and_store():
        speech_bytes = text_to_speech(f"Die Rechnung für {invoice.customer['name']} über {invoice.amount['total']} Euro wurde erstellt.")
